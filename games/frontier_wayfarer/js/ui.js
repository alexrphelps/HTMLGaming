(function (ns) {
    ...
    class UI {
        constructor() {
            ...
            this.panelTitle = document.getElementById('panelTitle'); this.message = document.getElementById('message'); this.activeTab = 'station'; this.game = null;
        ...
        togglePanel(game, tab) { if (this.panel.classList.contains('active')) this.closePanel(); else this.openPanel(game, tab); }
        ...
        updateHud(game) {
            ...
            document.getElementById('objective').innerHTML = active ? `<strong>${active.name}</strong><span>${active.description} // ${active.progress}/${active.required}</span>` : `<strong>FREE FLIGHT</strong><span>${ns.Unlocks.nextMilestone(state)}</span>`;
        ...
        render_station(game) {
            ...
            return `<div class="station-hero"><p class="eyebrow">${FACTIONS[station.faction].short} FACILITY</p><h2>${station.name}</h2><p>${ns.Unlocks.nextMilestone(state)}</p></div><div class="card-grid three">
                ...
                <div class="station-actions"><button data-action="tab" data-id="contracts">CONTRACT BOARD</button>${ns.Unlocks.evaluate(state).tradeTier ? '<button data-action="tab" data-id="trade">OPEN MARKET</button>' : ''}<button class="primary" data-action="undock">UNDOCK</button></div>`;
        }
        ...
        render_navigation(game) { const state = game.state; return `<div class="map-layout"><div class="galaxy-map">${REGIONS.map(r => `<div class="map-region" style="left:${(r.x + 11200) / 224}%;top:${(r.y + 3000) / 116}%;width:${r.w / 224}%;height:${r.h / 116}%;border-color:${r.color}"><span>${r.name}</span></div>`).join('')}${LANDMARKS.map(l => `<i class="map-point ${state.discoveries.includes(l.id) ? 'known' : ''}" title="${l.name}" style="left:${(l.x + 11200) / 224}%;top:${(l.y + 3000) / 116}%;background:${FACTIONS[l.faction].color}"></i>`).join('')}<i class="map-player" style="left:${(state.ship.x + 11200) / 224}%;top:${(state.ship.y + 3000) / 116}%"></i></div><aside><span class="eyebrow">SEAMLESS CHART</span><h2>${game.region.name}</h2><p>${state.discoveries.length} signals logged<br>${state.visitedRegions.length}/5 regions visited</p><h3>NEXT LICENSE</h3><p>${ns.Unlocks.nextMilestone(state)}</p></aside></div>`; }
        render_settings(game) { const s = game.state.settings; return `<div class="settings-list"><label><span>MASTER VOLUME</span><input id="volumeSetting" type="range" min="0" max="1" step="0.1" value="${s.volume}"></label><label><span>SCREEN SHAKE</span><button data-action="toggle-shake">${s.screenShake ? 'ENABLED' : 'DISABLED'}</button></label><button data-action="save">SAVE CAREER NOW</button><button class="danger" data-action="reset-career">DELETE CAREER</button><div class="controls"><h3>FLIGHT CONTROLS</h3><p>Mouse aim // W/S forward/reverse // A/D strafe</p><p>Mouse 1 weapon one // Mouse 2 weapon two // F interact/dock</p><p>Space/Q/E/Shift active modules // Tab target // M map // T traits // C contracts</p></div></div>`; }
        openDefeat(game, result) { this.activeTab = 'defeat'; this.panel.classList.add('active'); document.getElementById('panelTabs').innerHTML = ''; document.getElementById('menuWallet').innerHTML = this.walletHtml(game.state, false); this.panelTitle.textContent = 'RECOVERY REPORT'; this.panelBody.innerHTML = `<section class="defeat"><span class="eyebrow">SHIP DISABLED // RESCUE COMPLETE</span><h2>Recovered at ${result.station.name}</h2><p>All unbanked resources were lost.</p><div>${ns.Wallet.KEYS.map(key => `<span>${CREDIT_META[key].short} LOST <b>${result.lostResources[key]}</b></span>`).join('')}<span>CARGO LOST <b>${Object.values(result.lostCargo).reduce((a, b) => a + b, 0)}</b></span></div><button class="primary" data-action="recover">CONTINUE CAREER</button></section>`; }
        handleClick(event) {
            ...
            else if (action === 'buy-module' && ns.Economy.buyModule(state, id)) { game.save(); this.renderPanel(game); }
