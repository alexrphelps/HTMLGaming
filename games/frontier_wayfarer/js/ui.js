(function (ns) {
    ...
    class UI {
        ...
        updateHud(game) {
            ...
            const drive = ns.LightSpeed.status(game), driveHud = document.getElementById('lightDriveHud'); driveHud.className = `light-drive-hud ${drive.className}`; driveHud.innerHTML = `<span>R</span><strong>ASTERION</strong><small>${drive.label}<em>5 SS + 5 HE</em></small>`;
        ...
        moduleStats(module) {
            ...
            if (module.id === 'light_drive') values.push('<span>ACTIVATION COST<b>5 SS + 5 HE</b></span>', '<span>LIGHT SPEED LOAD<b>5 ENERGY + 5 HEAT / S</b></span>');
            return values.join('');
        }
