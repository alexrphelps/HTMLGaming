(function (ns) {
    ...
    class UI {
        constructor() {
            ...
            this.panelTitle = document.getElementById('panelTitle'); this.headerUndock = document.getElementById('headerUndock'); this.message = document.getElementById('message'); this.activeTab = 'station'; this.focusedContractId = null; this.selectedShipArea = 'engine'; this.shipMissionTab = 'weapons'; this.navigationView = 'local'; this.selectedGalaxyId = null; this.game = null; this.stargateCooldownTicker = 0;
        ...
        showMessage(text) { const convoyWarning = /^(CONVOY (?:HULL CRITICAL|SEPARATION WARNING)|ESCORT LINK CRITICAL)/.test(text); this.message.textContent = text; this.message.classList.toggle('convoy-warning', convoyWarning); this.message.classList.remove('show'); void this.message.offsetWidth; this.message.classList.add('show'); }
        ...
        openPanel(game, tab) { this.game = game; game.cancelInteraction?.(true); const nextTab = tab || this.activeTab; if (nextTab !== 'trade') this.recentlyBoughtModules.clear(); this.activeTab = nextTab; game.paused = true; this.panel.classList.add('active'); this.renderPanel(game); }
        ...
        render_station(game) {
            ...
            return `<div class="station-hero" style="--faction:${FACTIONS[station.faction].color}"><p class="eyebrow">${this.factionBadge(station.faction)} FACILITY // ${ns.Expansion.patrolStatus(state, station.faction)} ACCESS</p><h2>${station.name}</h2><p class="station-flair">${flair[Math.abs(station.x + station.y) % flair.length]}</p><small>${FACTIONS[station.faction].description}</small><div class="station-facts"><span>TRAFFIC<b>${details.traffic}</b></span><span>RUMOR<b>${details.rumor}</b></span><span>POLICY<b>${details.policy}</b></span></div><p>${ns.Unlocks.nextMilestone(state)}</p></div><div class="card-grid three">
        ...
        contractStageHtml(state, contract, stage, activeView) {
            ...
            return `<article class="contract-stage ${stage.status}" style="--faction:${faction.color}"><span>${this.factionBadge(factionId, stage.status.toUpperCase())}</span><strong>${stage.name || destination}</strong><small>${stage.status === 'active' && activeView ? `${destination} // ${remaining}` : destination}</small></article>`;
        }
        ...
        moduleStats(module) {
            ...
            if (module.ability) values.push(`<span>COOLDOWN<b>${module.ability.cooldown}S</b></span>`, `<span>DURATION<b>${module.ability.duration || 0}S</b></span>`, `<span>ACTIVE ENERGY<b>${module.ability.energy}</b></span>`);
