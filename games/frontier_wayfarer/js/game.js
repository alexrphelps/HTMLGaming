(function (ns) {
    ...
    class Game {
        constructor(canvas, ui) {
            ...
            this.state = null; this.world = null; this.region = ns.Data.REGIONS[0]; this.camera = { x: 0, y: 0 };
            this.bullets = []; this.enemies = []; this.time = 0; this.running = false; this.paused = false; this.last = 0; this.accumulator = 0;
        ...
        useState(state) {
            ...
            this.camera.x = state.ship.x; this.camera.y = state.ship.y; this.bullets = []; this.enemies = [];
        ...
        update(dt) {
            ...
            if (this.input.consume('m')) this.ui.togglePanel(this, 'navigation');
            if (this.input.consume('t')) this.ui.togglePanel(this, 'traits');
            if (this.input.consume('c')) this.ui.togglePanel(this, 'contracts');
            ns.Abilities.update(this, dt); this.updateShip(dt); this.updateBullets(dt); this.updateEnemies(dt); this.updateContract(dt);
            const previousRegion = this.region.id; this.region = this.world.update(this.state.ship.x, this.state.ship.y);
            ...
            this.camera.x += (this.state.ship.x - this.camera.x) * Math.min(1, dt * 5); this.camera.y += (this.state.ship.y - this.camera.y) * Math.min(1, dt * 5);
            if (this.autosaveTimer >= 120 && this.enemies.length === 0) { this.save('AUTOSAVE COMPLETE'); this.autosaveTimer = 0; }
            this.encounterTimer -= dt;
            if (this.encounterTimer <= 0 && this.enemies.length === 0 && !this.state.dockedAt && Math.random() < .18 + this.region.danger * .09) {
        ...
        updateShip(dt) {
            ...
            if (s.x < bounds.minX || s.x > bounds.maxX) { s.x = clamp(s.x, bounds.minX, bounds.maxX); s.vx *= -.35; if (this.messageTimer <= 0) this.notify('NAVIGATION LIMIT // TURN BACK'); }
            if (s.y < bounds.minY || s.y > bounds.maxY) { s.y = clamp(s.y, bounds.minY, bounds.maxY); s.vy *= -.35; if (this.messageTimer <= 0) this.notify('NAVIGATION LIMIT // TURN BACK'); }
            ...
            this.world.nearbyEntities(s.x, s.y, 70).filter(e => e.kind === 'asteroid').forEach(e => { if (circlesOverlap({ x: s.x, y: s.y, radius: 13 }, e)) { ns.Combat.applyDamage(this.state, 16 * dt); s.vx *= -.3; s.vy *= -.3; } });
        ...
        onDefeat() { const result = ns.Combat.defeatConsequences(this.state, this.world); this.enemies = []; this.bullets = []; this.paused = true; this.save(); this.ui.openDefeat(this, result); }
