(function (ns) {
    ...
    function objectDefinition(entity) { return ns.Data.WORLD_OBJECT_TYPES[entity?.typeId] || null; }
    function scenarioDefinition(entity) { return ns.Data.WORLD_SCENARIOS[entity?.typeId] || null; }
    ...
    function interact(game, entity) {
        ...
        if (entity.typeId === 'smuggler_dead_drop' && entity.variant === 'guarded') {
            game.spawnEnemies(2 + Math.floor(danger / 3), entity); game.notify(`DEAD DROP OPEN // ${rewardText(reward)} // RAIDER RESPONSE`);
        } else if (entity.typeId === 'unstable_prism') game.notify(`PRISM SURVEY LOGGED // ${rewardText(reward)} // SYSTEM LOAD SPIKE`);
        else game.notify(`${definition.name.toUpperCase()} SECURED // ${rewardText(reward)}`);
        return true;
    }
    ...
    function activate(game, entity) {
        ...
        if (entity.typeId === 'distress_call') {
            if (distressIsFalse(game, entity, definition)) { game.spawnEnemies(Math.min(4, 1 + Math.ceil(danger / 2)), entity); game.notify('FALSE DISTRESS SIGNAL // BANDIT AMBUSH'); }
            else { const reward = { aetherium: 45 + danger * 8, sunshards: 1, helionite: 0 }, standing = positiveStanding(game, 'independents', 1); ns.Wallet.credit(game.state, reward); ns.State.addExperience(game.state, 28); game.notify(`PILOT RECOVERED // +${reward.aetherium} AE // +${standing} GUILD STANDING`); }
        } else if (entity.typeId === 'border_skirmish') {
            game.spawnEnemies(2, { x: entity.x - 140, y: entity.y }, 'concord', 'world-event', 'concord_patrol');
            game.spawnEnemies(2, { x: entity.x + 140, y: entity.y }, 'corsairs', 'world-event', 'corsair_raider');
            game.notify('BORDER SKIRMISH // CONCORD AND CORSAIR CONTACTS');
        } else if (entity.typeId === 'raider_sweep') { game.spawnEnemies(Math.min(4, 1 + Math.ceil(danger / 2)), entity); game.notify('RAIDER SWEEP // HOSTILES INBOUND'); }
        else if (entity.typeId === 'abandoned_worksite') game.notify('ABANDONED WORKSITE // RECOVERABLE STORES DETECTED');
        ns.State.addExperience(game.state, entity.typeId === 'abandoned_worksite' ? 8 : 0); game.save(); game.ui.renderAll(game); return true;
    }
