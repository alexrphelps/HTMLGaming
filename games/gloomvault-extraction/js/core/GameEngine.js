class GameEngine {
    ...
    generateFloor(isNextFloor) {
        ...
        this.mapCols = this.currentMapConfig.cols;
        this.mapRows = this.currentMapConfig.rows;
        console.log(`Generating map style: ${this.currentMapKey}`);
        ...
        if (this.mapGen.bossRoom) {
            this.spawnBossRoomEntities(effectiveFloorLevel);
        }

        this.spawnFloorGuardian(startPos, effectiveFloorLevel);
        ...
        for (let p of doorPositions) {
            this.tryAddFloorTransition(p.x, p.y, 'door');
        }
        ...
        for (let p of holePositions) {
            this.tryAddFloorTransition(p.x, p.y, 'hole');
        }

        this.spawnDungeonServices(startPos);

        this.ensureDungeonExit(startPos);
    }
    ...
    update(dt) {
        ...
        for (let i = this.projectiles.length - 1; i >= 0; i--) {
            ...
            if (proj.isPlayerOwned) {
                ...
                for (let e of this.enemies) {
                    ...
                    if (Math.hypot(e.x - proj.x, e.y - proj.y) < (e.width/2 + proj.width/2)) {
                        if (proj.abilityEffect === 'element_bomb') {
                            hit = true;
                            break;
                        }
                        const damageMultiplier = this.getDamageTakenMultiplier(e);
                        const finalDamage = Math.round(proj.damage * damageMultiplier);
                        const damageInfo = e.takeDamage(finalDamage);
                        let actualDamage = Math.max(0, (damageInfo && damageInfo.hp ? damageInfo.hp : 0) + (damageInfo && damageInfo.shield ? damageInfo.shield : 0));
                        this.applyElementalHit(e, proj);
                        if (e.handleDamagedByPlayer) {
                            e.handleDamagedByPlayer(this, proj, actualDamage);
                        } else {
                            e.hasTakenPlayerDamage = true;
                            e.isAggroed = true;
                        }
                        ...
                        this.combatFeedback.addText(`-${finalDamage}`, e.x, e.y, proj.color || '#ffffff', 14, 0.8);
                        this.particleSystem.emitImpact(e.x, e.y, proj.color || '#ffffff');

                        // Lifesteal (melee gets bonus)
                        let lsCap = (typeof CombatConfig !== 'undefined' ? CombatConfig.caps.lifesteal : 0.35) + (this.player.stats.lifestealCapBonus || 0) + (this.player.soulSiphonCapBonus || 0);
                        let lifestealRate = (this.player.stats.lifesteal || 0) + (this.player.soulSiphonLifesteal || 0);
                        ...
                        if (proj.weaponVariant === 'overcharged' && Math.hypot(e.x - this.player.x, e.y - this.player.y) <= 105) {
                            e.staggerTimer = Math.max(e.staggerTimer || 0, proj.staggerDuration || 0.45);
                        }
                ...
                if (Math.hypot(this.player.x - proj.x, this.player.y - proj.y) < (this.player.width/2 + proj.width/2)) {
                    ...
                    this.applyElementalHit(this.player, proj);
                    this.camera.shake(8, 0.2); // Bigger shake for player taking damage
                    if (proj.owner && proj.owner.handleHitPlayer) {
                        proj.owner.handleHitPlayer(this, damageInfo, proj);
                    }
            ...
            if (hit || proj.markedForDeletion) {
                this.handleAbilityProjectileEnd(proj);
