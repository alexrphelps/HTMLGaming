class Projectile extends Entity {
    constructor(x, y, angle, speed, damage, lifetime, isPlayerOwned = true, weaponType = 'pistol', options = {}) {
        super(x, y, 10, 10, 1);
        this.angle = angle;
        this.speed = speed;
        ...
        this.color = options.color || (this.isPlayerOwned ? '#ffff00' : '#ff3333');
        this.chargeMultiplier = options.chargeMultiplier || 1;
        this.weaponVariant = options.weaponVariant || null;
        this.attackName = options.attackName || null;
        this.staggerDuration = options.staggerDuration || 0;
        
        this.isMelee = (weaponType === 'melee_stab' || weaponType === 'melee_cleave');
        this.pierce = this.isMelee || (this.isPlayerOwned && this.element === 'shadow');
        this.hitEnemies = new Set(); // track already-hit enemies for pierce

        if (this.element === 'arcane') {
            const sizeBonus = Math.round((this.chargeMultiplier - 1) * 14);
            this.width += sizeBonus;
            this.height += sizeBonus;
        }
    ...
    update(dt, mapGen, particleSystem) {
        ...
        if (particleSystem) {
            if (this.weaponType === 'melee_stab' || this.weaponType === 'melee_cleave') {
                particleSystem.emitDashTrail(this.x, this.y, this.color || '#ffffff');
            } else if (this.element) {
                particleSystem.emitDashTrail(this.x, this.y, this.color);
            }
        }
    ...
    render(ctx, renderer) {
        ...
        ctx.fillStyle = this.color || (this.isPlayerOwned ? '#ffff00' : '#ff0000');
        ctx.beginPath();
