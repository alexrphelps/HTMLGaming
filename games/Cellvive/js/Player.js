...
class Player {
    constructor(options = {}) {
        ...
        this.maxSprintEnergy = 100;
        this.sprintDrainRate = 0.5;
        this.sprintRegenRate = 0.3;
        ...
        this.regenRate = 0;
        this.passiveGrowthRate = 0;
        this.damageReduction = 0;
        this.toxicDamageReduction = 0;
        this.speedPenaltyReduction = 0;
        this.sporeAttractionRadius = 0;
        this.sporeAttractionStrength = 0;

        // Tier 1
        this.hasEfficientMetabolism = false;
        this.hasThickMembrane = false;
        this.hasRapidMovement = false;

        // Tier 2
        this.hasPhotosynthesis = false;
        this.hasMagneticOrganelle = false;
        this.hasAdaptiveSize = false;
        this.hasToxicResistance = false;

        // Tier 3
        this.hasPredatoryInstinct = false;
        this.hasCellularDivision = false;
        this.hasSymbioticShield = false;
        this.hasEvolutionaryLeap = false;

        // Symbiotic Shield (phase shift) state
    ...
    update(input, zoom = 1.0) {
        ...
        // Regen sprint energy when not sprinting
        if (!this.isSprinting) {
            this.sprintEnergy = Math.min(this.maxSprintEnergy, this.sprintEnergy + this.sprintRegenRate);
        }
    ...
    handleMovement(input) {
        ...
        const defaultInput = { left: false, right: false, up: false, down: false, shift: false };
        const safeInput = {};
        ...
        // Sprint logic - shift key multiplies speed while draining energy
        this.isSprinting = false;
        const isMovingInput = safeInput.left || safeInput.right || safeInput.up || safeInput.down;
        if (safeInput.shift && this.sprintEnergy > 0 && isMovingInput) {
            this.isSprinting = true;
            currentMaxSpeed *= this.sprintMultiplier;
            this.sprintEnergy = Math.max(0, this.sprintEnergy - this.sprintDrainRate);
        }
    ...
    regenerateHealth() {
        if (this.health < this.maxHealth) {
            let regen = this.healthRegen;
            if (this.hasPhotosynthesis && this.regenRate > 0) {
                regen += this.regenRate * (1 / 60);
            }
    ...
    handleGreenSpore(spore) {
        ...
        // Apply efficient metabolism talent
        if (this.hasEfficientMetabolism && this.growthMultiplier && growthAmount > 0) {
            growthAmount *= this.growthMultiplier;
            GameLogger.debug(`🧬 Efficient Metabolism: Growth multiplied by ${this.growthMultiplier}`);
        }
    ...
    updateMaxSpeed() {
        ...
        // Apply adaptive size talent - reduces speed penalty from size
        if (this.hasAdaptiveSize && this.speedPenaltyReduction > 0) {
            const speedPenalty = 1 - sizeFactor;
            const reducedPenalty = speedPenalty * (1 - this.speedPenaltyReduction);
            sizeFactor = 1 - reducedPenalty;
        }

        // Apply rapid movement talent
        ...
        this.maxSpeed = baseSpeed * sizeFactor * talentSpeedMultiplier;
    ...
    takeDamage(amount) {
        ...
        if (this.hasSymbioticShield) {
            this.checkPhaseShiftTrigger();
        }
        
        return false;
    }

    checkPhaseShiftTrigger() {
        ...
        if (this.health <= this.maxHealth * this.phaseShiftTriggerHealth &&
            ...
            GameLogger.debug(`🌀 Symbiotic Shield activated!`);
        }
    }
    ...
    updateTalentEffects(deltaTime, game) {
        // Update magnetic organelle attraction
        if (this.hasMagneticOrganelle) {
            this.updateMagneticOrganelle(game);
        }

        // Update symbiotic shield (phase shift)
        if (this.hasSymbioticShield) {
            this.updatePhaseShift(deltaTime);
        }

        // Cellular division - passive growth
        if (this.hasCellularDivision && this.passiveGrowthRate > 0) {
            this.updateCellularDivision();
        }
    }

    updateCellularDivision() {
        const targetRadius = Math.min(this.maxRadius, this.baseRadius + this.passiveGrowthRate * 0.01);
        if (targetRadius > this.baseRadius) {
            this.targetRadius = targetRadius;
            this.growthStartRadius = this.baseRadius;
            this.growthAnimationProgress = 0;
            this.isGrowing = true;
    ...
    /**
     * Update symbiotic shield - handles timer expiry
     */
    updatePhaseShift(deltaTime) {
        ...
        if (this.isPhaseShifted &&
            ...
            this.isPhaseShifted = false;
            GameLogger.debug(`🌀 Symbiotic Shield faded`);
        }
    }
    
    /**
     * Update magnetic organelle talent - attracts spores
     */
    updateMagneticOrganelle(game) {
    ...
    getRenderProps() {
        return {
            ...
            // Talent visual effects
            hasMagneticOrganelle: this.hasMagneticOrganelle,
            sporeAttractionRadius: this.sporeAttractionRadius
        };
    }
}
