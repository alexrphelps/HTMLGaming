...
class TutorialManager {
    ...
    getNearbyElements(player) {
        ...
        if (this.game.environmentManager) {
            ...
            if (!envElements || typeof envElements !== 'object') {
                return discoveries;
            }
            
            const biomes = Array.isArray(envElements.biomes) ? envElements.biomes : [];
            const foodSpawners = Array.isArray(envElements.foodSpawners) ? envElements.foodSpawners : [];
            const hazards = Array.isArray(envElements.hazards) ? envElements.hazards : [];
            const currents = Array.isArray(envElements.currents) ? envElements.currents : [];
    ...
    renderTutorialVisual(elementType, tutorial) {
        ...
        switch (elementType) {
            ...
            case 'biome_aggressive':
                ...
                break;
            case 'obstacle':
                this.renderFoodSpawnerVisual(ctx, centerX, centerY);
                break;
            case 'hazard_toxin':
                if (CELLVIVE_CONSTANTS.ENVIRONMENT.HAZARDS.ENABLED) {
     ...
     */
    renderFoodSpawnerVisual(ctx, centerX, centerY) {
        const size = 30;
