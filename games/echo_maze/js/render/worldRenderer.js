(function () {
  ...
  function drawCellFloor(ctx, state, x, y, camX, camY) {
    ...
    ctx.fillStyle = lit
      ? `rgba(18, 34, 45, ${0.50 + lightFade * 0.38})`
      : 'rgba(8, 13, 18, 0.72)';
    ...
    if (lit && em.rand01(state, x, y, 9090) < 0.12) {
      ctx.fillStyle = `rgba(125, 249, 255, ${0.05 + em.rand01(state, x, y, 9093) * 0.12})`;
      ctx.beginPath();
      ctx.arc(
  ...
  function drawCellWalls(ctx, state, x, y, camX, camY) {
    ...
    ctx.lineCap = 'square';
    ctx.strokeStyle = `rgba(140, 210, 230, ${alpha})`;
    ctx.shadowColor = lit ? 'rgba(125, 249, 255, 0.18)' : 'transparent';
  ...
  function drawCompass(ctx, state, w, h) {
    ...
    ctx.fillText(obj.name + ': ' + Math.round(distCells) + ' cells', x, y + 36);
    ctx.restore();
  }
  ...
  function drawMinimap(ctx, state, w, h) {
    ...
    const x0 = w - size - pad;
    const y0 = pad;
    const range = compact ? 28 : em.CONFIG.minimapRange;
    ...
    for (let y = pc.y - range; y <= pc.y + range; y++) {
      for (let x = pc.x - range; x <= pc.x + range; x++) {
        ...
        if (item && em.isLitCell(state, x, y)) {
    ...
    if (state.warden) {
      ...
      if (state.revealed.has(em.keyOf(wc.x, wc.y))) {
